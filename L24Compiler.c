/*
 * L24 complier program implemented in C
 * L24编译器（完整版本）
...
enum symbol {
  ...
  comma, semicolon, becomes, endsym, ifsym, 
  thensym, whilesym, elsesym, printsym, scansym, 
  mainsym, lbrace, rbrace,
};
#define symnum 28
...
void enter( int* ptx, int lev, int* pdx);
...
void block(int lev, int tx, bool* fsys)         
{
  ...
  if(sym==mainsym){
	...
	cx0 = cx;
	gen(ini, 0, 0);	            /* 生成指令，此指令执行时在数据栈中为被调用的过程开辟0个单元的数据区,不知道数量之后回填 */
  ...
  nxtlev[semicolon] = true;
	nxtlev[endsym] = true;
  nxtlev[elsesym] = true;
	statement(nxtlev, &tx, lev,&dx);      /*语句处理*/
  table[tx0].size = dx;	        /* 每有一条新的赋值指令就会多加一点dx，现在还不知道，要等程序结束后回填，dx是标识符数量加3 */
  code[cx0].a = dx;             /*这里进行回填*/
...
void statement(bool* fsys, int* ptx, int lev, int* pdx)
{
	...
	if (sym == ident)	/* 准备按照赋值语句处理 */
	{
		...
		if (i == 0)
		{
			vardeclaration(ptx, lev, pdx);  	/* 声明新标识符 */
		}
      getsym();
    i = position(id, *ptx);
	...
	else
	{
		if (sym == scansym)	/* 准备按照scan语句处理 */
		{
          ...
          i = position(id, *ptx);
					...
					if (i == 0)
					{
						error(35);	/* scan语句括号中的标识符应该是声明过的变量 */
					}
					else
					{
					...
					getsym();
				} while (sym == comma);	/* 一条scan语句可读多个变量 */
			}
		...
		else
		{
            ...
            nxtlev[lparen] = true;
            nxtlev[rparen] = true;
            nxtlev[endsym] = true;	/* 后继符号为then else ( ) end*/
            if(sym==lparen){
              getsym();
            ...
            if(sym==elsesym){
              ...
              code[cx2].a = cx;	/* 经statement处理后，cx为then后语句执行完的位置，它正是前面未定的跳转地址，此时进行回填 */
            }else{
						  code[cx1].a = cx;	/* 经statement处理后，cx为then后语句执行完的位置，它正是前面未定的跳转地址，此时进行回填 */
            }
              ...
              error(17);/*缺少end*/
					...
					else
					{
						if (sym == lbrace)	/* 准备按照复合语句处理 */
						{
							...
							memcpy(nxtlev, fsys, sizeof(bool) * symnum);
							nxtlev[semicolon] = true;
							nxtlev[rbrace] = true;	/* 后继符号为分号或右大括号 */

							statement(nxtlev, ptx, lev,pdx);  /* 对{与}之间的语句进行分析处理 */
	...
	while (inset(sym, facbegsys)) 	/* 循环处理因子 */
	{
		if (sym == ident)	/* 因子为常量或变量 */
		{
			...
			else
			{
				...
				gen(lod, 0, table[i].adr);	/* 找到变量地址并将其值入栈 */
